...
for(n in grep(".", DATA$pgn)){
  ...
  for(i in 1:length(pgn)){
    ...
    positions[i, "FEN"] <- to.FEN(position_df=positions[1:i,1:64])
  }; rm(i)
  DATA[n,"FEN"] <- positions[nrow(positions),"FEN"]
  print(n)
}; rm(n, pgn)
```
...
**Issue 5**
Function *mobility_piece* does not return complete list of options.
Link: o_7
```{r xmpl-pos}
DATA <- read.csv("data/chess.com IDs 1-1000.csv", stringsAsFactors = F)
positions <- setup()
...
# the following should include b1, but does not
mobility_piece(square="b3", position_vec=positions)
```
